  ...
  return (
    <div className="min-h-screen relative w-full flex bg-[#F8F9FD] font-sans animate-fade-in overflow-x-hidden pb-16">
      <div className="flex flex-col lg:flex-row w-full max-w-[1300px] mx-auto pt-8 lg:pt-10 pb-12 gap-8 px-4 lg:px-8">
        {/* === SIDEBAR === */}
        <aside
          className="hidden lg:flex flex-col w-full lg:w-[260px] shrink-0 gap-6 h-fit lg:sticky lg:top-10 animate-fade-in-up"
        ...
        >
          ...
          <nav className="hidden lg:flex flex-col gap-2 relative z-10 w-full mt-2">
            ...
            >
              <LayoutDashboard
                className={`w-[22px] h-[22px] ${activeTab === "dashboard" ? "text-white/50" : ""}`}
              />{" "}
              Dashboard
            </button>
            <button
            ...
            >
              <Car
                className={`w-[22px] h-[22px] ${activeTab === "vehicles" ? "text-white/50" : ""}`}
              />{" "}
              Vehicles
            </button>
            <button
            ...
            >
              <Fuel
                className={`w-[22px] h-[22px] ${activeTab === "quotas" ? "text-white/50" : ""}`}
              />{" "}
              Quotas
            </button>
            <button
            ...
            >
              <History
                className={`w-[22px] h-[22px] ${activeTab === "history" ? "text-white/50" : ""}`}
              />{" "}
              History
            </button>
          </nav>
        ...
        <main className="flex-1 flex flex-col gap-8 w-full min-w-0">
          <header
            className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-2 animate-fade-in-up"
              ...
              <div className="flex flex-col md:items-end justify-center">
                ...
                <div className="bg-white rounded-full pl-3 pr-5 py-2.5 shadow-[0_10px_20px_-10px_rgba(0,0,0,0.05)] flex items-center gap-4 min-h-[56px] min-w-[200px] border border-slate-50 transition-all hover:border-slate-200 relative cursor-pointer">
                  ...
                  <div className="w-[38px] h-[38px] rounded-full bg-[#E8FFF3] flex items-center justify-center text-[#2E8B57] shrink-0">
                    {vehicle ? (
                      getVehicleIcon(vehicle.fuel_type || vehicle.vehicle_type)
                    ) : (
          ...
          <div className="flex flex-col gap-10 items-center w-full min-w-0 mt-8">
            ...
            {showQuota && vehicle && vehicle.status !== "rejected" && (
              <div
                className="bg-white w-full max-w-3xl rounded-[2.5rem] p-8 lg:p-10 shadow-[0_10px_30px_-15px_rgba(0,0,0,0.05)] relative overflow-hidden transition-all duration-500 animate-scale-in"
                style={{ animationDelay: "0.1s", animationFillMode: "both" }}
              >
                <div className="flex justify-between items-start mb-8">
                  <div>
                    <h3 className="text-[22px] font-black text-[#11153D] tracking-tight flex items-center gap-2">
                      <span className="bg-[#4285F4] text-white px-2 py-0.5 rounded-md text-[20px] leading-tight font-black">
                        Weekly
                      </span>{" "}
                      Quota
                    </h3>
            ...
            {showHistory &&
              (!vehicle ||
                ...
                history.length > 0) && (
                <div
                  className="w-full max-w-4xl mt-4 animate-fade-in-up"
                  style={{ animationDelay: "0.1s", animationFillMode: "both" }}
                >
            ...
            {showFuelPass && vehicle && vehicle.status === "approved" && (
              <div
                className="bg-[#11153D] w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden animate-scale-in"
                style={{ animationDelay: "0.1s", animationFillMode: "both" }}
              >
            ...
            {/* PENDING APPROVAL CARD (Dashboard & Vehicles Tabs) */}
            {(showFuelPass || activeTab === "vehicles") &&
              vehicle &&
              vehicle.status === "pending" && (
                <div
                  className="bg-amber-50 w-full max-w-sm rounded-[2.5rem] p-8 shadow-[0_10px_30px_-15px_rgba(0,0,0,0.05)] border border-amber-200 relative overflow-hidden animate-scale-in"
                  style={{ animationDelay: "0.2s", animationFillMode: "both" }}
                >
                  <div className="flex flex-col items-center text-center gap-4">
                    <div className="w-[64px] h-[64px] rounded-full bg-amber-100 flex items-center justify-center text-amber-600 shrink-0 mb-2">
                      <svg
                        width="32"
                        height="32"
                        viewBox="0 0 24 24"
                    ...
                    <h3 className="text-[20px] font-black text-amber-800 tracking-tight">
                      Approval Pending
                    </h3>
                    <p className="text-[13px] font-bold text-amber-600/80 leading-relaxed px-2">
                      Your vehicle registration is currently under manual
                      review. Please wait for administrative approval to access
                      your FuelPass and Quota.
                    </p>
                  </div>
                </div>
              )}

            {/* REJECTED APPROVAL CARD (Dashboard & Vehicles Tabs) */}
            {(showFuelPass || activeTab === "vehicles") &&
              vehicle &&
              vehicle.status === "rejected" && (
                <div
                  className="bg-red-50 w-full max-w-sm rounded-[2.5rem] p-8 shadow-[0_10px_30px_-15px_rgba(0,0,0,0.05)] border border-red-200 relative overflow-hidden animate-scale-in"
                  style={{ animationDelay: "0.2s", animationFillMode: "both" }}
                >
                  <div className="flex flex-col items-center text-center gap-4">
                    <div className="w-[64px] h-[64px] rounded-full bg-red-100 flex items-center justify-center text-red-600 shrink-0 mb-2">
                      <XCircle
                        className="w-[32px] h-[32px]"
                        strokeWidth={2.5}
                      />
                    </div>
                    <h3 className="text-[20px] font-black text-red-800 tracking-tight">
                      Application Rejected
                    </h3>
                    <p className="text-[13px] font-bold text-red-600/80 leading-relaxed px-2">
                      Your vehicle registration request was rejected by an
                      administrator. Please contact support or register again
            ...
            {activeTab === "vehicles" &&
              ...
              vehicle.status === "approved" && (
                <div
                  className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-[0_10px_30px_-15px_rgba(0,0,0,0.05)] relative overflow-hidden animate-scale-in"
                  style={{ animationDelay: "0.2s", animationFillMode: "both" }}
                >
                  <div className="flex items-center gap-4 mb-6">
                    <div className="w-[42px] h-[42px] rounded-[13px] bg-[#E8EAEF] flex items-center justify-center text-[#11153D] shrink-0">
                      {vehicle ? (
                        getVehicleIcon(
                          vehicle.fuel_type || vehicle.vehicle_type,
                        )
                      ) : (
                        <Car className="w-[20px] h-[20px]" />
                      )}
                  ...
                  <div className="flex flex-col gap-5">
                    <div className="flex flex-col">
                      <span className="text-[9px] font-black text-[#A1A5B7] uppercase tracking-widest mb-1">
                        Fuel Type
                      </span>
                      <span className="font-bold text-[#11153D] text-[14px]">
                        {vehicle.fuel_type || vehicle.vehicle_type}
                      </span>
                    </div>
                    <div className="flex flex-col">
                      ...
                      <span className="font-black font-mono text-[#4285F4] text-[13px]">
                        {vehicle.chassis_number || "N/A"}
                      </span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[9px] font-black text-[#A1A5B7] uppercase tracking-widest mb-1">
                        Registered Owner (NIC)
                      </span>
                      <span className="font-bold text-[#11153D] text-[14px] uppercase tracking-wide">
                        {vehicle.nic_number || "N/A"}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            {showRegisterForm && (
              <div
                className="bg-[#F4F5F8] w-full max-w-xl mx-auto rounded-[2.5rem] p-10 pt-11 border border-[#EAECEF] animate-fade-in-up"
                style={{ animationDelay: "0.1s", animationFillMode: "both" }}
              >
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.08)] z-50 px-6 py-4 flex justify-between items-center pb-safe">
        ...
        >
          <div
            className={`p-2 rounded-2xl ${activeTab === "dashboard" ? "bg-[#F4F5F8]" : ""}`}
          >
        ...
        >
          <div
            className={`p-2 rounded-2xl ${activeTab === "vehicles" ? "bg-[#F4F5F8]" : ""}`}
          >
        ...
        >
          <div
            className={`p-2 rounded-2xl ${activeTab === "history" ? "bg-[#F4F5F8]" : ""}`}
          >
        ...
        >
          <div
            className={`p-2 rounded-2xl ${activeTab === "quotas" ? "bg-[#F4F5F8]" : ""}`}
          >
